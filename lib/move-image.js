 ...
 */
var moveImage = function (image, leftChange, topChange, height, width) {
      var minTopPosition = - (image.height - height)
        , minLeftPosition = - (image.width - width)

        , leftPosition = parseInt(image.style.left.slice(0, -2), 10) + leftChange
        , topPosition = parseInt(image.style.top.slice(0, -2), 10) + topChange
